			...
			if (sender instanceof HumanEntity) {
				...
				if (args.length == 0) {
					...
					if (args[0].equals("new")) {
						if (player.hasPermission("genericmc.claim")) {
							...
							sender.sendMessage("You do not have permission to claim land.");
							return true;
						}
						...
						if (player.hasPermission("genericmc.claim")) {
							if (args.length < 2) { // Check that there is a name
								...
								if (member.hasPerm(RolePerms.CLAIM)) { // Check that this player has perms
									...
									sender.sendMessage(
											"You do not have permission within your faction to unclaim land.");
									return true;
								}
							}
						} else {
									...
									if (claim != null) { // Check the claim exists
										if (claim.numChunks() > 1) {
											...
											if (claim.removeChunk(chunk)) {
												// Remove the chunk if it's in the claim, or if it's not then do
												// nothing.
												sender.sendMessage("Removed chunk (" + chunk.getX() + ", "
														+ chunk.getZ() + ") from claim: " + claim.getName());
												GenericPlugin.saveData(GenericPlugin.getPlugin());
												return true;
											} else {
